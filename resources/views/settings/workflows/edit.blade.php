<x-app-layout>
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-2xl font-semibold text-gray-900">Edit: {{ $workflow->name }}</h1>
    </div>

    <form method="POST" action="{{ route('app.settings.workflows.update', $workflow) }}" x-data="wfForm({{ json_encode($workflow->steps ?? []) }})">
        @csrf @method('PUT')
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $workflow->name) }}" required class="border border-gray-300 rounded-md px-3 py-2 text-sm w-full focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Min Amount</label>
                            <input type="number" name="min_amount" value="{{ old('min_amount', $workflow->min_amount) }}" min="0" step="0.01" class="border border-gray-300 rounded-md px-3 py-2 text-sm w-full focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Max Amount</label>
                            <input type="number" name="max_amount" value="{{ old('max_amount', $workflow->max_amount) }}" min="0" step="0.01" class="border border-gray-300 rounded-md px-3 py-2 text-sm w-full focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                    </div>
                    <div>
                    ...
                    <div class="space-y-3">
                        <template x-for="(step, i) in steps" :key="i">
                            <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                                ...
                                <select :name="`steps[${i}][approver_type]`" x-model="step.approver_type" class="border border-gray-300 rounded px-2 py-1 text-sm">
                                    <option value="role">By Role</option>
                                    <option value="user">Specific User</option>
                                </select>
                                <template x-if="step.approver_type === 'role'">
                                    <select :name="`steps[${i}][role]`" x-model="step.role" class="border border-gray-300 rounded px-2 py-1 text-sm flex-1">
                                ...
                                <template x-if="step.approver_type === 'user'">
                                    <select :name="`steps[${i}][user_id]`" x-model="step.user_id" class="border border-gray-300 rounded px-2 py-1 text-sm flex-1">
                                        @foreach($users as $user)
                                        <option value="{{ $user->id }}">{{ $user->name }}</option>
                                        @endforeach
                                    </select>
                                </template>
                                <button type="button" @click="removeStep(i)" x-show="steps.length > 1" class="text-red-400 hover:text-red-600 text-lg">×</button>
                            </div>
                        </template>
                    </div>
                </div>
            ...
            <div class="space-y-4">
                <button type="submit" class="w-full px-4 py-2 bg-blue-700 text-white text-sm font-medium rounded-lg hover:bg-blue-800">Update Workflow</button>
    ...
    @push('scripts')
    <script>
    function wfForm(initialSteps) {
        return {
            steps: initialSteps && initialSteps.length ? initialSteps : [{ approver_type: 'role', role: 'department_head', user_id: '' }],
